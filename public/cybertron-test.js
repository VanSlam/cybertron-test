(function () {
    ...
    ***************************************************************************** */
    ...
    function level3() {
      ...
      const exercise1 = _ => {
        return cars.map(car => capitalizeWords(car.model))
      };

      const ex2 = 'Use filter to return a list of cars made between 2001-2004';
      const exercise2 = _ => {
        return cars.filter(car => car.year > 2000 && car.year < 2005)
      };

      const ex3 =
        'Use reduce to count the number of cars that were made in the 2000s';
      const exercise3 = _ => {
        return cars.reduce((acc, car) => (car.year > 2000 ? acc + 1 : acc), 0)
      };
      ...
      const exercise5 = _ => {
        const prices = cars.map(car => car.salesPrice);
        return prices.map(car =>
          car.toLocaleString('en-US', { style: 'currency', currency: 'USD' })
        )
      };
